...
// define control scheme: Simple || PID
#define Simple
...
// 0 is counting up, 1 is checking
volatile byte timerstatel = 0; 
...
int forward(long distance) {
  ...
  #ifdef Simple 
    int error = 0;
    int speedRight = speedForward;
    int speedLeft = speedForward + 37;
    int count = 0;
    motorRight.writeMicroseconds(speedRight);
    motorLeft.writeMicroseconds(speedLeft);
    while (1) {
...
void encoderHandlerRight() {
  ...
  encoderCountRight++;
    ...
    if (timerstater) {
      tR = TCNT3;
      if (tR > (320 * 4)) // cut off for our high frequnecy filter. Chosen somewhat arbitrarily.
        encoderSpeedRight = ((double)(1 / ((double)tR))) * 16000 * 4; //speed in ticks/sec
...
void encoderHandlerLeft() {
  ...
  encoderCountLeft++;
    ...
    if (timerstatel) {
      tL = TCNT4;
      if (tL > (320 * 4))
        encoderSpeedLeft = ((double)(1 / ((double)tL))) * 16000 * 4; //speed in ticks/sec
